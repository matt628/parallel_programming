...
# ╔═╡ b7d36955-9648-4caa-bd18-d8018658092f
md"""
### Wybrany algorytm oraz środowisko testujące

Do testów wybrałem algorytm nr 1. Testowałem go na moim prywatnym komputerze: MacBook Pro z procesorem M1Pro z 8 fizycznymi rdzeniami. Każdy pomiar powtórzyłem trzykrotnie.
"""
...
##### fragment sprawozdania wspólny z: Patryk Wojtyczek
...
W teorii program powinien działać najszybciej dla rozmiaru kubełka równego 1. Zgodnie z sugestią podaną na laboratorium, przetestowałem wszystkie rozmiary od 1 do 100 w wersji sekwencyjnej, aby sprawdzić, jak zachowywał się będzie algorytm w naszej implementacji. Jako rozmiar problemu przyjąłem $$10^6$$
"""
...
md""" 
Na powyższym wykresie możemy zaobserwować, że w okolicach rozmiaru problemu 25, całkowity czas wykonania stabilizuje się. Możemy również zaobserwować, że pod koniec badanego przedziału widoczny jest powolny wzrost. Dodatkowo, dla rozmiaru kubełka powyżej 50, zasadniczą część sortowania stanowi sortowanie kubełków, co nie jest zgodne z ideą algorytmu sortowania kubełkowego. Wybrałem rozmiar 10, ponieważ dla tego rozmiaru czas wykonania jest zadowalający, a jednocześnie jest to przed "skrzyżowaniem" się czasów sortowania i rozdzielenia do kubełków.
"""
...
Po określeniu optymalnego rozmiaru kubełka, uruchomiłem algorytm w wersji równoległej, dla liczby wątków od 1 do 8.
"""


# ╔═╡ 97de3a11-a310-4935-9b0e-921168fd6edd
begin 
    alg_data = CSV.read("results/algorithm/res_1.tsv", DataFrame; delim=";")
...
Poniższy wykres przedstawia czas wykonania dla różnych wątków. Widzimy, że najwięcej czasu pochłania rozdział danych do kubełków, co jest pożądane. Dodatkowo, widzimy że czas wykonania maleje wraz ze wzrostem liczby wątków, co jest jeszcze bardziej pożądane.
"""
...
begin
	...
	plot(ylabel="Czas wykonania [s]", xlabel="threads", xticks=0:1:8, legend = :topright, title="Czas wykonania w zależności od liczby wątków")
...
Na poniższym wykresie umieszczone zostało przyspieszenie poszczególnych etapów z idealnym przyspieszeniem zaznaczonym niebieską linią.
Można zaobserwować, że jedynym etapem, który zrównolegla się niemal doskonale jest generowanie danych.

Etapem, który również zrównolegla się stosunkowo dobrze, jest zapisywanie posortowanych kubełków. W algorytmie pierwszym, każdy wątek zapisuje swoje kubełki do innego fragmentu tablicy, co redukuje konkurowanie o dostęp do zasobów.

Problematycznymi etapami są sortowanie kubełków oraz rozdzielanie danych do kubełków. 

Najtrudniejszy do zrównoleglenia okazał się etap sortowania kubełków. Było to dla mnie zaskoczenie, ponieważ spodziewałem się, że etap ten będzie się zrównoleglał niemal doskonale.


Kolejnym problematycznym etapem było rozdzielenie danych do kubełków. Trudności ze zrównolegleniem tego etapu prawdopodobnie operają się o konieczość przejścia przez całą tablicę każdego z wątku. Co prawda implementacyjne staraliśmy się zabezpieczyć wątki przed rywalizacją o dostęp do tablicy, jednak nadal każdy wątek musi przeczytać dane, a zyskujemy jedynie na tym, że możemy równolegle zapisywać do wielu kubełków.
...
Ten wykres nasuwa podobne wnioski do poprzednich - generowanie i zapis mają część sekwencyjną bliską zeru, a sortowanie i podział na kubełki wyraźnie większą.

Zgodnie z prawem Amdahla, część sekwencyjna jest czynnikiem determinującym ograniczenie przyspieszenia, co możemy zaobserwować na powyższym oraz poniższym wykresie. 
"""
...
# ╔═╡ da993dfa-b460-4f3c-81de-22b7d5819fce
md"""
### Część 4
...
begin
	...
	plot(ylabel="Czas wykonania [s]", xlabel="threads", xticks=0:1:8, legend = :topright, title="Generating speedup")
...
begin
	plot(ylabel="Czas wykonania [s]", xlabel="threads", xticks=0:1:8, legend = :topright, title="Splitting speedup")
...
begin
	plot(ylabel="Czas wykonania [s]", xlabel="threads", xticks=0:1:8, legend = :topright, title="Sorting speedup")
...
begin
	plot(ylabel="Czas wykonania [s]", xlabel="threads", xticks=0:1:8, legend = :topright, title="Writing speedup")
...
begin
	plot(ylabel="Czas wykonania [s]", xlabel="threads", xticks=0:1:8, legend = :topright, title="Overall speedup")
